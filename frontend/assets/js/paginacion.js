 ...
 */
function renderizarPaginacion({ respuesta, idContenedor, onPageChange, onPerPageChange, perPage = 10 }) {
  ...
  const opcionesPerPage = [10, 25, 50, 100].map(val => 
    `<option value="${val}" ${perPage == val ? 'selected' : ''}>${val} / pág</option>`
  ).join('');
